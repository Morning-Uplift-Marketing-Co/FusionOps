...
import { generateAstroProject } from "../../utils/astro-generator.jsx";
import { getTemplate } from "#lp-template-generator/core/template-registry.js";
...
export function TemplateGeneratorModal({ open, onClose, onSave, templates }) {
    ...
    const goNext = () => {
        if (state.step < getSteps().length - 1) {
            // Auto-generate files for clone mode if user skipped "Generate from This"
            if (state.mode === MODE_FROM_TEMPLATE && state.step === 0 && !state.generatedFiles && state.sourceTemplate) {
                const tmpl = getTemplate(state.sourceTemplate);
                if (tmpl) {
                    const sampleSite = {
                        brand: "SampleBrand", domain: "example.com", colorId: "blue", fontId: "dm-sans",
                        h1: "Sample Headline", sub: "Sample subheadline", cta: "Get Started",
                    };
                    const files = tmpl.generate(sampleSite);
                    const sourceCode = `// Cloned from templates/${state.sourceTemplate}\n// New template ID: ${state.newFolderId || 'my-template'}\n// Generated: ${new Date().toISOString()}`;
                    setState(prev => ({ ...prev, generatedCode: sourceCode, generatedFiles: files, step: prev.step + 1 }));
                    return;
                }
            }
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            // Generate actual Astro project files so the template is usable
            const astroSite = {
                brand: state.templateName || "My Template",
                colorId: state.colorId || "blue",
                fontId: state.fontId || "dm-sans",
                loanType: state.category,
                cta: state.hasHeroForm ? "Get Started" : "Learn More",
                sub: state.templateDescription || "Custom template generated by Template Wizard.",
                domain: `${state.newFolderId || "example-template"}.com`,
                h1: state.templateName || "My Template",
            };
            const generatedFiles = generateAstroProject(astroSite);
            const templateCode = generateTemplateCode(state);
            setState(prev => ({ ...prev, generatedCode: templateCode, generatedFiles, step: 4 }));
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Keep modal open so user can see the error / retry
        } finally {
            setIsGenerating(false);
        }
    };
    ...
    const canGoNext = () => {
        if (state.mode === MODE_FROM_TEMPLATE) {
            ...
            // On review step, allow save if either code or files exist
            return state.generatedCode !== null || state.generatedFiles !== null;
        }
        ...
        if (state.mode === MODE_FROM_ZIP) {
            ...
            // On review step, allow save if files were parsed from ZIP
            return state.generatedCode !== null || state.generatedFiles !== null;
        }
        switch (state.step) {
