...
const RECENT_TXN = [
    { name: "Kenneth Thompson", email: "ken99@yahoo.com", amount: 316, status: "Success" },
    { name: "Abraham Lincoln", email: "abe45@gmail.com", amount: 242, status: "Success" },
    { name: "Monserrat Rodriguez", email: "monserrat44@gmail.com", amount: 837, status: "Processing" },
    { name: "Silas Johnson", email: "silas22@gmail.com", amount: 874, status: "Success" },
    { name: "Carmella DeVito", email: "carmella@hotmail.com", amount: 721, status: "Failed" },
];
...
export function Dashboard({ sites, stats, ops, setPage, startCreate, settings = {}, apiOk, neonOk }) {
    ...
    const filteredTxn = RECENT_TXN.filter(t =>
        !txFilter || t.name.toLowerCase().includes(txFilter.toLowerCase()) || t.email.toLowerCase().includes(txFilter.toLowerCase())
    );
                            ...
                            <TableBody>
                                {filteredTxn.map((t, i) => (
                                    <TableRow key={i}>
                                        ...
                                        <TableCell className="text-[hsl(var(--muted-foreground))]">{t.email}</TableCell>
                                        <TableCell className="text-right font-mono">${t.amount.toFixed(2)}</TableCell>
                                        <TableCell>
                                            <span className={`text-xs font-medium px-2.5 py-1 rounded-md ${statusCls[t.status] || ""}`}>{t.status}</span>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
