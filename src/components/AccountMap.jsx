    ...
    const loadMapData = async () => {
        ...
        try {
            ...
            const mapped = fetchedCards.map(card => {
                ...
                // Proxy info (from profile or account)
                const proxyHost = profile?.proxyHost || link?.proxyIp || "";
                const proxyPort = profile?.proxyPort || "";
                const proxyUser = profile?.proxyUser || "";
                const proxyLabel = proxyHost ? `${proxyHost}${proxyPort ? ':' + proxyPort : ''}` : "No Proxy";
                ...
                return {
                    ...
                    proxy: { host: proxyHost, port: proxyPort, user: proxyUser, label: proxyLabel },
                    metrics: {
                        spend: cardSpend,
                        trueCost,
                        revenue,
                            ...
                            <Card key={acc.id} className="overflow-hidden hover:shadow-md transition-all border-[hsl(var(--border))]">
                                ...
                                <CardContent className="p-0">
                                    <div className="grid grid-cols-2 divide-x divide-y divide-[hsl(var(--border))] text-sm">
                                        ...
                                        {/* Proxy & Card Status Bottom */}
                                        <div className="col-span-2 p-3 text-xs flex items-center gap-2 text-[hsl(var(--muted-foreground))] border-b border-[hsl(var(--border))] bg-[hsl(var(--card))]">
                                            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${acc.proxy.host ? 'bg-emerald-500' : 'bg-red-500'}`} />
                                            <span className="font-mono text-[11px] text-[hsl(var(--foreground))]">
                                                {acc.proxy.label}
                                            </span>
                                            {acc.proxy.user && (
                                                <span className="text-[10px] opacity-50">({acc.proxy.user})</span>
                                            )}
                                        </div>
