...
import { validateAstroStandard } from "../../utils/template-standard.js";
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const astroSite = {
                ...
                sub: state.templateDescription || "Custom Astro template generated by Template Wizard.",
                domain: `${state.newFolderId || "example-template"}.com`,
                conversionId: state.includeTracking ? (state.conversionId || "AW-PLACEHOLDER") : "",
                voluumDomain: state.includeTracking ? (state.voluumDomain || "tracking.example.com") : "",
                formStartLabel: state.includeTracking ? "form_start" : "",
                formSubmitLabel: state.includeTracking ? "form_submit" : "",
                aid: state.aid || "",
                h1: state.templateName || "My Template",
                amountMax: state.amountMax || 5000,
            };
            const generatedFiles = generateAstroProject(astroSite);
            const validation = validateAstroStandard(generatedFiles);
            const templateCode = generateTemplateCode(state);
            setState(prev => ({
                ...prev,
                generatedCode: templateCode,
                generatedFiles,
                templateFormat: validation.format || "astro",
