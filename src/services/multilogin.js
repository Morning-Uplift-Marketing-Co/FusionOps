...
/**
 * Assign a proxy to a Multilogin profile.
 * @param {string} profileId  MLX profile UUID
 * @param {string} folderId   MLX folder UUID
 * @param {{ host, port, user, pass, type }} proxy  Proxy config
 */
async function assignProxyToProfile(profileId, folderId, proxy) {
    ...
    const proxyPayload = {
        ...
        parameters: {
            proxy: {
                type: proxy.type === "residential" ? "HTTP" : proxy.type === "isp" ? "HTTP" : "HTTP",
                host: proxy.host,
    ...
    return updateProfile(proxyPayload);
}
...
export const multiloginApi = {
    ...
    // Proxy assignment
    assignProxyToProfile: (profileId, folderId, proxy) => assignProxyToProfile(profileId, folderId, proxy),
