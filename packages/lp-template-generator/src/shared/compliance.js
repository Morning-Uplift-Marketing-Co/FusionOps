...
export function legalModalsHTML(config) {
  ...
  <div class="popup-overlay" id="contactPopup"><div class="popup-card"><button class="popup-close" onclick="closePopup('contactPopup')">\u2715</button><div class="popup-header"><h3>\ud83d\udcde Contact Us</h3></div><div class="popup-body" style="text-align:center"><strong style="font-size:16px">${config.brand}</strong><p>\ud83d\udccd ${config.address}</p><p>\ud83d\udcde <span style="color:var(--primary);font-weight:700">${config.phone}</span></p><p>\u2709\ufe0f <a href="mailto:${config.email}" style="color:var(--primary)">${config.email}</a></p></div></div></div>
