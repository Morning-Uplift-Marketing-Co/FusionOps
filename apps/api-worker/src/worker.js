...
// ═══════════════════════════════════════════════════════════════════════════
// Google Ads / Analytics tracking relay (Option G — see
// .planning/specs/tracking-pipeline-v1/DESIGN.md). Browser LPs fetch gtag.js
// and beacon conversions via t.{domain} so ad blockers + ITP cannot strip
// them. Relay scope is deliberately narrow: strictly allowlisted upstream
// paths, conversion-ID regex gate, no arbitrary open-proxy behavior.
//
// Feature-flagged with env.ENABLE_GTAG_RELAY = "1". Off by default so the
// code can ship ahead of LP-side inject-tracking changes without affecting
// production. When disabled, the routes 404 silently and gtag.js calls fall
// back to direct Google endpoints (zero LP breakage).
// ═══════════════════════════════════════════════════════════════════════════
...
function isGtagRelayEnabled(env) {
  const v = String(env?.ENABLE_GTAG_RELAY || '').trim().toLowerCase();
  return v === '1' || v === 'true' || v === 'yes' || v === 'on';
}

function gtagRelayUpstreamKeyForPath(path) {
...
// path unknown). 404 is only returned for allowlisted paths that fail input
// validation — callers should treat null as "not this route".
async function handleGtagRelay(request, url, env) {
