...
const features = [
  {
    icon: DollarSign,
    title: `Installment Loans ${amountMin} – ${amountMax}`,
  ...
  },
  {
    ...
    description: `Qualified applicants on select plans can access 0% APR financing. Rates range from ${aprMin}% up to ${aprMax}% APR.`,
  },
  {
    icon: Clock,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-5">
            {features.map(({ icon: Icon, title, description }, i) => (
              <div
                key={title}
                className={`group flex gap-4 p-5 rounded-xl border border-gray-100 hover:border-teal-200 hover:bg-teal-50/40 transition-all duration-300 ${shouldAnimate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
